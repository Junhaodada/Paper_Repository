...
class Model(Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            # 4*84*84 => 32*H_out*W_out
            # H_out = floor((84-7-1)/4+1) = 20
            # W_out = floor((84-7-1)/4+1) = 20
...
class QFuncloss(Module):
    def __init__(self, gama: float) -> None:
        super().__init__()
        self.gama = gama
    ...
    def forward(self, q: torch.Tensor, action: torch.Tensor, double_q: torch.Tensor,
        ...
        q_sampled_action = q.gather(-1,
                                    action.to(torch.long).unsqueeze(-1).squeeze(-1))
        with torch.no_grad():
            best_next_action = torch.argmax(double_q, -1)
            best_next_q_value = target_q.gather(-1,
                                                best_next_action.unsqueeze(-1).squeeze(-1))
            q_update = reward + self.gama * best_next_q_value * (1 - done)
